...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_POST['query-context']))	$context = stripcleantohtml( $_POST['query-context'] );

	//$to = 'info@spart.pro';
	$to = 'admin@spart.pro';
	$subject = 'Сайт: ЗАПРОС НАЛИЧИЯ';
	$message = '<h1 class="header">Запрос наличия</h1>';

	if( $customer != '' )	$message .= '<p class="customer">Имя: <strong>' . $customer . '</strong></p>';
	if( $contacts != '' )	$message .= '<p class="contacts">Контакты: <strong>' . $contacts . '</strong></p>';
	if( $additional != '' )	$message .= '<p class="additional">Запрос: <strong>' . $additional . '</strong></p>';
	if( $context != '' )	$message .= '<p class="context">Искал на сайте: <strong>' . $context . '</strong></p>';
